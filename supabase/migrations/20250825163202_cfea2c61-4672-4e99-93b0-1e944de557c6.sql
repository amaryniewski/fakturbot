-- Naprawianie ostrzeżeń bezpieczeństwa KSeF
...
-- 2. Usunięcie security_barrier z widoku zmaterializowanego (warning 6)
-- Materialized views nie powinny być dostępne przez API
ALTER MATERIALIZED VIEW ksef_user_stats SET (security_barrier = false);

-- 3. Naprawa widoku ksef_auto_fetch_status - usunięcie security definer property
-- Ponowne utworzenie widoku bez security definer
...
-- 4. Dodanie RLS do widoku zmaterializowanego przez funkcję dostępu
-- Zamiast polegać na RLS materialized view, utworzymy funkcję dostępu
CREATE OR REPLACE FUNCTION get_user_ksef_stats(p_user_id UUID DEFAULT NULL)
RETURNS TABLE (
    user_id UUID,
